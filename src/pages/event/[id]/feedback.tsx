...
const Index = () => {
  ...
  const isLoading =
    session.status === "loading" || eventLoading || feedbackLoading;
      ...
      {isLoading ? (
        ...
        <div className="flex w-full max-w-3xl flex-col items-center gap-2">
          ...
          <Link
            ...
            href={`/${event?.creator?.creatorProfile ?? ""}`}
          >
            See more events and courses by {event?.creator?.name ?? ""}{" "}
        ...
        >
          <h1 className="text-center text-xl md:text-2xl lg:text-left lg:text-3xl">
            Feedback for the{" "}
            <span className="font-medium">{event?.title ?? ""}</span> event
          </h1>
